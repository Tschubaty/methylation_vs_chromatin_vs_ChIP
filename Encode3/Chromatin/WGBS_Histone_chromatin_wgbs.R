...
#   foreach (chr_n = 1:length(CHR_NAMES ), .combine = rbind) %dopar% {
    ...
    #  for(r in 200000:210000){
      ...
      #r <- 1
      if(r%% 1000 == 0){
        print(paste("CpG row number:",r,"chromatin row number" ,i,"percent = ",i/nrow(df)*100,sep=" "))
      }
      ########################### K562 ##########################
      # check if need to go to next segment
      while (df$end[r] > df_chromatin$end[i] & i <= nrow(df_chromatin)) {
        i <- i+1
      }
      
      # if there is annotation in segement
      if(df_chromatin$start[i] <= df$start[r]){
          #DO ANNOATTE
        df[r,c("chromatin_state_HepG2","chromatin_state_K562")] <- df_chromatin[i,c("chromatin_state_HepG2","chromatin_state_K562")]
      }
