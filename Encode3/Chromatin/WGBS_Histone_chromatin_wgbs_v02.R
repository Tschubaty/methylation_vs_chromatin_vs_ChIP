...
#   foreach (chr_n = 1:length(CHR_NAMES ), .combine = rbind) %dopar% {
    ...
    df <- df[200000:210000,]
    dt_chromatin <- as.data.table(df_chromatin)
    dt <- as.data.table(df)
    print(nrow(dt))
    i <- 1
    ...
    for(r in 1:nrow(dt)){
      ...
      #r <- 1
      if(r%% 1000 == 0){
        print(paste("CpG row number:",r,"chromatin row number" ,i,"percent = ",i/nrow(dt)*100,sep=" "))
      }
      ########################### K562 ##########################
      # check if need to go to next segment
      while (dt$end[r] > dt_chromatin$end[i] & i <= nrow(dt_chromatin)) {
        i <- i+1
      }
      
      # if there is annotation in segement
      if(dt_chromatin$start[i] <= dt$start[r]){
          #DO ANNOATTE
        set(dt,i = r ,j = c("chromatin_state_HepG2","chromatin_state_K562") , value=dt_chromatin[i,c("chromatin_state_HepG2","chromatin_state_K562")])
      }
    ...
    ######################close(pb)
    saveRDS(object = as.data.frame(dt),file = file.path(output_folder,sub(pattern = ".rds",replacement = ".chromatin.RDS",x =  file_name)))
