...
file_name <- "ctcf_motifs_groups.csv"
file_path <- file.path(input_folder, file_name)

# Load the data
data <- read.csv(file_path)

# Select the row with the highest distribution value for each experiment
highest_distribution_data <- data %>%
  group_by(Experiment) %>%
  slice_max(order_by = Distribution, n = 1, with_ties = FALSE) %>%
  ungroup()

# Print the resulting data
print(highest_distribution_data[,c(1,4,5,6,7,10,12)])
