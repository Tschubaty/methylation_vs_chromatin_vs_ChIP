...
base_path = r"D:/Users/Daniel Batyrev/Documents/GitHub/CTCF"
...
# Loop over all folders (biosamples)
for root, dirs, files in os.walk(base_path):
    for dir_name in dirs:
        if '_CTCF-human_' in dir_name:
            parts = dir_name.split('_')
            biosample = parts[0]
            protein = 'CTCF'
            experiment = parts[-1]
            inner_dir_path = os.path.join(root, dir_name)
            html_file_path = os.path.join(inner_dir_path, 'meme-chip.html')
            ...
            # Load the HTML file
            print(f"Loading HTML file for {biosample}...")
            ...
            try:
                ...
                print(f"Clicked 'Expand All Clusters' for {biosample}.")
            except Exception as e:
                print(f"Error clicking 'Expand All Clusters' for {biosample}: {e}")

            # Check if the HTML file is loaded
            if "404 Not Found" in driver.title:
                print(f"Error: {html_file_url} not found")
                continue

            # Extract the groups and motifs
            print("Extracting groups and motifs...")
            motif_boxes = driver.find_elements(By.CSS_SELECTOR, ".motifbox")
            print(f"Found {len(motif_boxes)} motif boxes in {biosample}.")
            ...
            print(f"Extracted motifs for {biosample}.")

            # Process the centrimo.html to extract concentration values
            centrimo_html_file_path = os.path.join(inner_dir_path, 'centrimo_out', 'centrimo.html')
            centrimo_html_file_url = 'file:///' + centrimo_html_file_path.replace("\\", "/")
            driver.get(centrimo_html_file_url)
            time.sleep(3)  # Adjust the sleep time if necessary
            ...
            for group in all_groups:
                if group['Protein'] == protein and group['Biosample'] == biosample and group['Experiment'] == experiment:
                    for centrimo in centrimo_data:
                        if centrimo["Motif ID"] == group["single Motif ID"]:
                            group["Distribution"] = centrimo["Concentration"]
                            # print(f"Updated concentration for motif {group['single Motif ID']} in group {group['Group ID']}: {centrimo['Concentration']}")
...
output_csv_path = "C:/Users/Daniel Batyrev/Documents/GitHub/methylation_vs_chromatin_vs_ChIP/Encode3/meme/meme-chip_results/ctcf_motifs_groups.csv"
