...
input_folder <- file.path(
  this.dir,
  "meme",
  "fimo_single_experiments_on_known_motif_CpG_and_methylation_and_peaks"
)
...
for (protein in protein_folders) {
  ...
  for (bed_file in files) {
    ...
    # Remove duplicates that are only different in score
    df <- df %>%
      distinct(chr, start, end, chr_peak, start_peak, end_peak, .keep_all = TRUE)
    
    # Count unique combinations of (chr_peak, start_peak, end_peak)
    n_peaks <- df %>%
      distinct(chr_peak, start_peak, end_peak) %>%
      nrow()
    
    # Identify if CpG is matched
    is_CpG_matched <- !df$chr == "."
    
    # Count unmatched peaks
    n_peaks_unmatched <- sum(!is_CpG_matched)
    
    # Count unique combinations of (chr, start, end) for matched CpG
    n_CpG <- df[is_CpG_matched, ] %>%
    ...
    summary_list[[length(summary_list) + 1]] <- data.frame(
      ...
      motif = motif,
      n_peaks = n_peaks,
      n_peaks_unmatched = n_peaks_unmatched,
      n_CpG = n_CpG
    )
  }
  ...
  df_protein <- df_protein %>%
    mutate(across(c(start, end, start_peak, end_peak, fRead), as.numeric), # Convert numeric-like columns to numeric
           across(
             c(
               chr,
               strand,
  ...
  df_protein <- df_protein %>%
    mutate(across(
      c(start, end, start_peak, end_peak),
      ~ ifelse(is.na(.), NA_integer_, as.integer(.))
  ...
  df_protein$chr_peak <- factor(df_protein$chr_peak, levels = CHR_NAMES)
  
  # Check the new levels
  #levels(df_protein$Chromatin_State)
  #levels(df_protein$chr)
  #levels(df_protein$chr_peak)
