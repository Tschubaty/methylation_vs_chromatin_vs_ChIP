...
base_dir <- "D:/Users/Daniel Batyrev/Documents/GitHub/homer/appended_bed"

# List all knownResults.txt files in the subdirectories
files <- list.files(path = base_dir, pattern = "knownResults.txt", recursive = TRUE, full.names = TRUE)

# Initialize an empty dataframe to store the best results
best_results <- data.frame()

# Function to read and process each knownResults.txt file
process_file <- function(file) {
  # Read the data
  data <- read_delim(file, delim = "\t", col_names = TRUE, trim_ws = TRUE)
  ...
  # Add the folder name to the dataframe for context
  best_motif$Folder <- basename(dirname(file))
...
for (file in files) {
  print(file)
  try(best_results <- bind_rows(best_results, process_file(file)))
...
write.csv(best_results, file = "best_motifs_results.csv", row.names = FALSE)
